...
export default function Register(props){
    ...
    let onSubmit = async() => {
        try {
            ...
            let checkEmail = await axios.get(`https://my-json-server.typicode.com/arifitantotito/jsonserver-1/users?email=${inputEmail}`)
            let checkUsername = await axios.get(`https://my-json-server.typicode.com/arifitantotito/jsonserver-1/users?username=${inputUsername}`)
            
            if(checkEmail.data.length === 0 && checkUsername.data.length === 0){
                // Post
                await axios.post('https://my-json-server.typicode.com/arifitantotito/jsonserver-1/users', {username: inputUsername, email: inputEmail, password: inputPassword})
